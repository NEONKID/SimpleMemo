...
public class RealmHelper {
    ...
    public void insertMemo(int id, String title, String content, String color, @Nullable MemoImage mi,
        ...
        // 기존 메모인 경우..
        if (editMemoWithId(id, title, content, color, mi, onSuccess, onError))
            return;
        ...
        realm.executeTransaction(realm -> {
            if (mi != null)
                memo.setTransactionImg(realm, mi);
            realm.copyToRealm(memo);
        });
    }

    /**
     * 메모에 이미지를 DB에 저장하는 동기 함수입니다.
     ...
     * @param mi 추가할 이미지
     */
    public void insertMemoImage(Realm realm, Memo memo, MemoImage mi) {
        if (mi.getId() == 0)
            mi.setId(getNewMemoImageId(realm));
     ...
     * 작성 순서별로 배치하기 위해 Primary Key룰 부여받은 순으로 정렬하며,
     *
     * 반드시 해당 클래스에서 동기 처리를 위해서만 사용해주십시오
     *
     * @param realm Realm Instance
     * @return 저장된 메모들
     */
    private RealmResults<Memo> getMemos(Realm realm) {
        return realm.where(Memo.class).findAll().sort(Memo.FIELD_ID, Sort.ASCENDING);
    }

    /**
     * 한 메모에 들어있는 모든 이미지를 가져오는 비동기 함수입니다.
     *
    ...
    /**
     * 한 메모에 들어있는지 모든 이미지를 가져오는 함수입니다.
     *
     * 반드시 해당 클래스에서 동기 처리를 위해서만 사용해주십시오
     *
     * @param realm Realm Instance
     ...
     * @return memoId에 해당하는 이미지들
     */
    private RealmResults<MemoImage> getMemoImages(Realm realm, int memoId) {
        return realm.where(MemoImage.class).equalTo("memo.id", memoId).findAll()
                .sort(MemoImage.FIELD_ID, Sort.ASCENDING);
    }

    /**
     * 메모의 ID 값에 해당하는 메모를 가져오는 비동기 함수입니다.
     *
     * @param id 대상 메모 ID
    ...
    /**
     * 메모의 ID 값에 해당하는 메모를 가져오는 동기 함수입니다.
     *
     * 반드시 해당 클래스에서 동기 처리를 위해서만 사용해주십시오.
     *
     * @param realm Realm Instance
     ...
     * @return id에 해당하는 모든 메모
     */
    private RealmResults<Memo> getMemoWithId(Realm realm, int id) {
        return realm.where(Memo.class).equalTo(Memo.FIELD_ID, id).findAll()
                .sort(Memo.FIELD_ID, Sort.ASCENDING);
    }

    /**
     * 메모 이미지의 ID 값에 해당하는 이미지를 가져오는 비동기 함수입니다.
     *
    ...
    /**
     * 메모 이미지의 ID 값에 해당하는 이미지를 가져오는 동기 함수입니다.
     *
     * 반드시 해당 클래스에서 동기 처리를 위해서만 사용해주십시오.
     *
     * @param realm Realm Instance
     ...
     * @return id에 해당하는 모든 메모 이미지
     */
    private RealmResults<MemoImage> getMemoImageWithId(Realm realm, int id) {
    ...
    private boolean editMemoWithId(int id, String title, String content, String color,
                                   @Nullable MemoImage mi, Realm.Transaction.OnSuccess success, Realm.Transaction.OnError error) {
        AtomicReference<RealmResults<Memo>> res = new AtomicReference<>(getMemoWithId(id));

        if (res.get().isEmpty())
            return false;

        realm.executeTransactionAsync(realm -> {
           res.set(getMemoWithId(realm, id));

           if (res.get().isValid()) {
               Memo memo = res.get().get(0);
    ...
    /**
     * 대상 메모를 제거하는 비동기 함수입니다.
     * 메모 이미지가 존재한다면, 이미지 제거 후, 삭제를 진행합니다.
     *
     * @param id 대상 메모 ID
     */
    public void deleteMemoWithId(int id,
                                 Realm.Transaction.OnSuccess success, Realm.Transaction.OnError error) {
        deleteMemoImageWithMemoId(id);

        realm.executeTransactionAsync(realm -> {
            RealmResults<Memo> res = getMemoWithId(realm, id);
    ...
    public void deleteMemoImageWithMemoImageId(int imageId,
        ...
        realm.executeTransactionAsync(realm -> {
            RealmResults<MemoImage> res = getMemoImageWithId(realm, imageId);
    ...
    private void deleteMemoImageWithMemoId(int memoId) {
        realm.executeTransactionAsync(realm -> {
            RealmResults<MemoImage> res = getMemoImages(realm, memoId);
    ...
    private int getNewMemoImageId(Realm realm) {
        return getAutoIncrementId(realm, new MemoImage());
    }
    ...
    private int getAutoIncrementId(Realm realm, Object obj) {
        Number curId = null;
